...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Soft fog for depth - pastel purple tint
    scene.fog = new THREE.FogExp2(0xf3e7ff, 0.002);
    ...
    // Lights (Dimmed for darker background)
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.4);
    scene.add(ambientLight);

    const pointLight = new THREE.PointLight(0x667eea, 0.8); // Soft Blue
    pointLight.position.set(5, 5, 5);
    scene.add(pointLight);

    const pointLight2 = new THREE.PointLight(0x764ba2, 0.8); // Soft Purple
    ...
    // Material - Glass-like / Iridescent (Water Droplet)
    const material = new THREE.MeshPhysicalMaterial({
        color: 0xeeeeee, // Slightly darker white
        roughness: 0.2,  // Increased roughness to reduce glare
        metalness: 0.1,
        transmission: 0.9,
        thickness: 1.5,
    ...
    for (let i = 0; i < particlesCount * 3; i++) {
        // Random positions around the blob
    ...
    // const noiseSpeed = 0.002; // Unused
    let noiseStrength = 0.3;

    // Click Interaction (Ripple/Pulse)
    document.addEventListener('click', () => {
        // Temporary increase in noise strength
        const originalStrength = 0.3;
        noiseStrength = 1.0;

        // Smoothly return to normal
        const restoreInterval = setInterval(() => {
            noiseStrength -= 0.05;
            if (noiseStrength <= originalStrength) {
                noiseStrength = originalStrength;
    ...
    function animate() {
        ...
        // Color Animation (Subtle shift)
        // const hue = (time * 0.1) % 360;
        // const color = new THREE.Color(`hsl(${hue}, 50%, 90%)`);

        pointLight.color.setHSL(Math.sin(time * 0.1), 0.5, 0.6);
        pointLight2.color.setHSL(Math.cos(time * 0.1), 0.5, 0.6);
