...
class AgeCalculatorUI {
    ...
    bindEvents() {
        ...
        this.bindCalendarTypeEvents();
    }

    /**
     * 양력/음력 토글 이벤트 처리
     */
    bindCalendarTypeEvents() {
        const radios = document.querySelectorAll('input[name="calendar-type"]');
        const lunarOptions = document.querySelector('.lunar-options');

        radios.forEach(radio => {
            radio.addEventListener('change', (e) => {
                if (e.target.value === 'lunar') {
                    lunarOptions.style.display = 'flex';
                } else {
    ...
    async calculateAgeAsync(isoBirthDate) {
        ...
        if (isoBirthDate) {
            ...
            if (calendarType === 'lunar') {
                try {
                    const year = parseInt(v.iso.substring(0, 4));
                    const month = parseInt(v.iso.substring(5, 7));
                    const day = parseInt(v.iso.substring(8, 10));
                    ...
                    finalDate = `${solarDate.year}-${String(solarDate.month).padStart(2, '0')}-${String(solarDate.day).padStart(2, '0')}`;

                    console.log(`음력 변환: ${year}-${month}-${day} (윤달:${isLeap}) -> 양력: ${finalDate}`);
                } catch (e) {
                    console.error('음력 변환 실패:', e);
                    // 변환 실패 시 사용자에게 알림 (선택 사항) 또는 그대로 전송
                }
            }

            formData.set('birth_date', finalDate);
        }
